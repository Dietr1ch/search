    ...
    C: Cost,
{
                ...
                match self.node_index.get(&s) {
                    ...
                    Some((node_index, false)) => {
                        ...
                        if new_g < neigh.g {
                            ...
                            self.open[neigh.heap_index.as_usize()].rank = neigh.rank(neigh_h);
    ...
    fn _unsafe_pop_non_trivial_heap(&mut self) -> usize {
        ...
        loop {
            ...
            debug_assert!(self.open[hole.as_usize()].rank <= self.open[child.as_usize()].rank);
        ...
        debug_assert_eq!(
            self.nodes[heap_node.node_index].heap_index.as_usize(),
            0,
    ...
    fn _unsafe_sift_up(&mut self, index: HeapIndex) {
        ...
        debug_assert_eq!(
            self.nodes[self.open[index.as_usize()].node_index].heap_index,
            index,
            "Node is out of sync."
        );
