...
impl<St: State, C: Cost> AStarHeapNode<St, C> {
    #[inline(always)]
    fn g(&self) -> C {
        self.rank.g()
    }
...
impl<St: State, C: Cost> PartialEq for AStarHeapNode<St, C> {
    #[inline(always)]
...
impl<St: State, C: Cost> Eq for AStarHeapNode<St, C> {}

/// PartialOrd is forwarded to Ord::cmp
impl<St: State, C: Cost> PartialOrd for AStarHeapNode<St, C> {
    #[inline(always)]
...
impl<St: State, C: Cost> Ord for AStarHeapNode<St, C> {
    #[inline(always)]
    ...
    C: Cost,
{
            ...
            for (s, a) in self.problem.space().neighbours(&state) {
                ...
                match self.nodes.get_mut(&s) {
                    ...
                    None => {
                        ...
                        self.push_new(&s, Some(state), neigh_g, neigh_h);
                    }
                }
            }
    ...
    #[inline(always)]
    #[must_use]
    fn pop(&mut self) -> Option<AStarHeapNode<St, C>> {
        #[cfg(feature = "coz_profile")]
        coz::scope!("Pop");
    ...
    #[inline(always)]
    fn push_new(&mut self, s: &St, parent: Option<St>, g: C, h: C) {
        self.verify_heap();
        ...
        // 2. Add AStarHeapNode to open using it's SearchTreeIndex
    ...
    fn _unsafe_pop_non_trivial_heap(&mut self) -> AStarHeapNode<St, C> {
        #[cfg(feature = "coz_profile")]
    ...
    pub fn write_memory_stats<W: std::io::Write>(&self, mut out: W) -> std::io::Result<()> {
        ...
        let s = size_of::<AStarHeapNode<St, C>>();
        let l = self.open.len();
